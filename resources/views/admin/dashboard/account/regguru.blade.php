            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Daftar Akun Guru</h3>
                </div><!-- /.box-header -->
                ...
                <div class="box-body">
                  <table id="dataGuru" class="table table-bordered table-hover">
                    ...
                    <tbody>
                     <?php foreach ($listAccountGuru as $itemAccount):  ?>
                      <tr>
                        <td>{{$itemAccount->guruNip}}</td>
                        <td>{{$itemAccount->guruNama}}</td>
                        <td>{{$itemAccount->guruEmail}}</td>
                        ...
                        <td>
                            ...
                            <a class="btn btn-success btn-flat btn-sm" onClick="tampilModal('{{$itemAccount->guruNip}}','{{$itemAccount->guruNama}}','{{$itemAccount->guruEmail}}','{{$itemAccount->guruKelasKode}}')" id="registerGuru{{{$itemAccount->guruId}}}" title="Register"><i class="fa fa-plus"></i></a>
                            @else
                           
                            <a class="btn btn-danger btn-flat btn-sm" href="{{{ URL::to('accountguru/'.$itemAccount->id.'/hapus') }}}" title="Hapus Akun"><i class="fa fa-trash"> Unreg </i></span>
                            </a> 
                            @endif
              ...
              <div class="modal-dialog">
                  <div class="modal-content">
                      <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel">Register Guru - Tambah</h4>
                      </div>
                      <div class="modal-body">
           
                          <form id="formRegisterGuru" class="form-horizontal" role="form" method="POST" action="{{ url('/accountguru/tambah') }}">
                              ...
                              <div class="form-group">
                                  ...
                                  <div class="col-md-6">
                                      <input type="text" class="form-control" id="kelasKode" name="kelasKode" readonly="true">
                                      <small class="help-block"></small>
                                  </div>
                              </div>               
                              <div class="form-group">
...
@section('script')
    ...
    <script type="text/javascript">
      $(function () {
        ...
        $('#dataGuru').DataTable({"pageLength": 50});

      });

      function tampilModal(nip,nama,email,kelasKode){
        //alert(level);
        $('input+small').text('');
        ...
        document.getElementById('kelasKode').value=kelasKode;
      ...
      $(document).on('submit', '#formRegisterGuru', function(e) {  
            ...
            .done(function(data) {
                ...
                window.location.href=url+'/accountguru'; 
            })
            .fail(function(data) {
                console.log(data.responeJSON);
                $.each(data.responseJSON, function (key, value) {
                    var input = '#formRegisterGuru input[name=' + key + ']';
